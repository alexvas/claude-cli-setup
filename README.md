...
- Linux или Windows через WSL2
- Docker Engine 24+ с BuildKit; в Windows включите интеграцию Docker Desktop с используемым дистрибутивом WSL2 (либо запускайте Docker Engine внутри WSL2)
- Python 3, установленный в среде Linux/WSL2

Нативный Windows Python и прямой запуск из PowerShell или командной строки Windows не поддерживаются. В Windows клонируйте репозиторий и храните кэши конструктора в Linux-файловой системе WSL2 (например, в `~/src` и `~/.cache`), а не в `/mnt/c`: проект полагается на Linux-семантику владельцев, прав доступа, блокировок, символических ссылок и атомарных файловых операций. Все команды конструктора запускайте из shell WSL2, используя Linux-пути.
...
Обычные текстовые сборки показывают нативный прогресс Docker/BuildKit по мере его появления. Для единственного машиночитаемого результата используйте `--output json`.
...
### Проект конструктора и локальный компаньон

Каждая команда выбирает один проект конструктора: текущий рабочий каталог по умолчанию или `--project-directory DIR`. Его фиксированная структура содержит `docker-constructor.toml`, необязательный `docker-constructor.local.toml`, `Dockerfile`, необязательный `.env` и `.docker-local/` непосредственно в этом каталоге. Локальный компаньон содержит только машинно-зависимое состояние и не может переопределять проверяемую политику, зависимости или `cache.ttl`.
...
`cache.ttl` относится к проверяемому `docker-constructor.toml`; `cache.dir` допускается только в `docker-constructor.local.toml`. `--no-cache` обходит HTTP-кэш для одной проверки обновлений, не изменяя проверяемый TTL.

Без `[cache].dir` постоянный корень — `${XDG_CACHE_HOME}/docker-constructor`, если `XDG_CACHE_HOME` непустой и абсолютный, иначе `~/.cache/docker-constructor`. HTTP-ответы находятся в `versioning/`; проверенные артефакты, locks и временное состояние — в `runtime-artifacts/blobs`, `runtime-artifacts/locks` и `runtime-artifacts/tmp`. Runtime-проекции и evidence по умолчанию хранятся во внешнем пространстве состояния проекта, ключом которого служит канонический путь выбранного проекта конструктора; неявный каталог `.docker-generated` не создаётся ни в проекте конструктора, ни в workspace.

Локальный `cache.dir` должен быть абсолютным выделенным корнем, принадлежащим конструктору. Нельзя выбирать `/`, домашний каталог, сам `XDG_CACHE_HOME` или его предка. Каталоги конструктора защищаются режимом `0700`, HTTP entries — `0600`, а проверенные blobs — `0444`; права существующих родителей, включая `XDG_CACHE_HOME`, не меняются. Если выбранный путь принадлежит другому пользователю или не может быть защищён, восстановите владельца либо удалите устаревшее поддерево конструктора и повторите команду. Доступ к хосту включать не требуется.
...
Откройте интерактивный выбор workspace:

```bash
./docker/docker-constructor.py run --tui
```

Выбранный основной workspace становится рабочим каталогом контейнера и монтируется 1:1 по тому же абсолютному пути. Дополнительные workspace монтируются 1:1 с последовательной нумерацией `WORKSPACE_PATH_2`, `WORKSPACE_PATH_3`, … без ограничения количества. Хостовый `~/.pi` монтируется в `/home/dev/.pi`. Корень дерева TUI можно задать через `WORKSPACE_ROOT` в `.env` выбранного проекта конструктора или `--workspace-root`.

Прямой запуск с явными workspace:
...
2. `--suggest` работает в режиме **non-mutating**: он выводит полный блок для ручной замены, но никогда не редактирует файл автоматически и не предназначен для добавления в конец TOML. В каноническом `docker-constructor.toml` найдите заголовок `# --- pi-tools.pi ---` и замените весь блок до следующего заголовка `# --- ... ---`. Фрагмент сохраняет неизменённые source, update-policy, override, validation и артефакты всех настроенных платформ; проверьте их вместе со значениями кандидата. Заголовки служат только для навигации; используйте полный путь TOML-таблицы для поиска проверяемой записи.
...
Для другого управляемого компонента найдите путь параметра в конфигурации, выполните `check-updates --only <path> --suggest` и замените полный предложенный блок, а не добавляйте отдельные поля. Проверьте сохранённые поля и артефакты всех настроенных платформ, затем запустите validate, просмотр diff, сборку и проверку. Для `runtime.pi-extensions` сборка обновляет итоговую конфигурацию версий образа, но не смонтированное состояние; обновите его в разделе «Обслуживание».
...
Стандартный текстовый отчёт предназначен для просмотра: `TARGET`, `PROVIDER`, `CURR -> NEXT`, `STATUS` и `PUBLISHED`. Он удаляет у цели только начальный префикс `build.stages.`, показывает дату публикации как `YYYY-MM-DD`, а пояснения provider выносит в раздел `Details:`. При интерактивном текстовом поиске stderr временно показывает `Checking updates [N/T] TARGET (PROVIDER)…`; в перенаправленном выводе progress-байтов нет.

Используйте `--details` для полной диагностической таблицы с полными путями, отдельными current/candidate, применимостью, пояснениями и временем публикации в формате `YYYY-MM-DD HH:MM:SS GMT`. Для автоматизации используйте `--json`: это машиночитаемый интерфейс, на который `--details` не влияет.

- **Интерактивный просмотр:** `--only <provider-or-path>` сужает поиск, `--suggest` добавляет полные non-mutating TOML-блоки для ручной замены.
