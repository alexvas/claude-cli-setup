...
### Корпоративное доверие и прикладной прокси

Корпоративные сетевые настройки являются локальными для машины: добавляйте `[corporate-trust]` и `[network.proxy]` только в выбранный локальный компаньон `docker-constructor.local.toml`, а сертификаты помещайте по фиксированному пути репозитория `.docker-local/corporate-ca-bundle.crt`. Пример:
...
Файл сертификатов — **полная замена** системного `/etc/ssl/certs/ca-certificates.crt`, а не дополнительный сертификат. Поэтому он должен содержать все публичные и корпоративные корневые сертификаты, нужные клиентам контейнера; конструктор проверяет только PEM framing и Base64, а валидность сертификатов и полнота доверия остаются ответственностью оператора.

URL прокси требует явные host и port и поддерживает `http`, `socks5` или `socks5h`. Необязательный `no_proxy` создаёт `NO_PROXY`/`no_proxy` только при явной настройке. Учётные данные прокси и URI userinfo не допускаются: используйте прокси без credentials. Поддержка SOCKS во время сборки best-effort: клиент сборки, не поддерживающий `socks5` или `socks5h`, может завершиться обычной ошибкой.

После включения или изменения bundle сертификатов доверие на стадии сборки требует rebuild image (пересборки). Перезапуск или новый launch монтирует текущий bundle read-only и получает изменения сертификатов, не требуя rebuild (пересборки); это не live reload уже работающего контейнера или процесса.

Эта функция не настраивает и не контролирует proxy/trust клиента или демона Docker, аутентификацию или доверие registry, image pulls и разрешение `FROM`. При необходимости оператор должен настроить эти внешние средства хоста и Docker отдельно.
