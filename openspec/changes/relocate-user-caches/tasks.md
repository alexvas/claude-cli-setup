...
- [ ] 2.1 **RED:** Add failing filesystem tests requiring `0700` on a new dedicated root and its constructor-created `versioning`, `runtime-artifacts`, blob, lock, and temporary directories, including creation of a missing explicit absolute `XDG_CACHE_HOME` with `0700`.
- [ ] 2.2 **RED:** Add failing tests requiring `0600` HTTP entries, `0444` verified blobs, no mode change to an existing `XDG_CACHE_HOME`, and no permission changes to any other resolved-root parent.
- [ ] 2.3 **RED:** Add failing filesystem tests accepting an existing writable XDG directory, rejecting an explicit absolute XDG non-directory or unwritable path without fallback, and using the `~/.cache` fallback only for empty or non-absolute XDG; require existing invoking-user-owned roots to be secured to `0700`, and symlinked selected roots, non-directory entries, foreign-owned directories, and unsecurable paths to be rejected through no-follow inspection before mutation.
