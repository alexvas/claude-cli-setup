...
- Let npm verify locked SRI when present and use its pinned registry path when an official lock omits SRI; explicitly record integrity-less registry nodes and independently validate the resulting closure, paths, identities, permissions, symlinks, omissions, and canonical tree manifest.
- Derive a stable assembler input identity from lock and policy inputs, but do not treat it as a content address. After validation, derive a distinct assembled output identity from the input identity, canonical output-tree digest, and canonical assembler-evidence digest; atomically publish immutable environments only under output identity and fully revalidate all three bindings on cache hits. Integrity-less registry nodes may produce different output identities from the same input identity.
...
- `user-cache-storage`: Adds a shared opaque npm download cache, immutable assembled-output storage keyed by content-derived output identity, and a non-authoritative input lookup index with explicit ownership and lifecycle boundaries.
