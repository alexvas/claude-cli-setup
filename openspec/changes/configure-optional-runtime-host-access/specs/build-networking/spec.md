...
**Reason**: Builds and unconditional launches do not require host access; machine-local gateway state now belongs to explicit Docker-gateway diagnostics and local TOML state.

**Migration**: Enable `[runtime.host-access]` in `docker-gateway` mode and run `doctor`; do not rely on build or `.env` to populate `HOST_GATEWAY_IP`.

### Requirement: Expose host mapping in direct Docker runs
**Reason**: Host mapping is no longer unconditional and is governed by the new runtime host-access policy.

**Migration**: Enable either Docker-gateway or external-address host access and configure its local address when `host.docker.internal` is required.
