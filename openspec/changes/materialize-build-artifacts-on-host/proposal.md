...
- Validate and publish the host-visible Pi tree and assembler evidence; construct `/opt/pi/bin/pi` explicitly from safe root `bin` metadata; record consumer evidence for its exact contents, mode, target, and containment; bind that evidence’s deterministic digest and assembler environment identity to a post-materialization transaction/build-plan attestation, not resolved build projection; include the environment and both evidence sets in the immutable build snapshot; and let BuildKit verify them against that materialized attestation before copying into `/opt/pi` with no npm networking or installation.
