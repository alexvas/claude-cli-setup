...
After host verification, invoke the standalone boundary delivered by `add-locked-npm-environment-assembler` with the official package and lock inputs. It runs the fixed script-free npm policy, returns a host-validated tree and evidence, and reuses the shared opaque assembler download cache. Construct `/opt/pi/bin/pi` explicitly from safe root `bin.pi` metadata, then record consumer evidence for the launcher’s exact contents, non-writable executable mode, resolved target, and containment within the Pi environment. After materialization, bind the assembler environment identity and deterministic consumer-evidence digest to a transaction/build-plan attestation, pass them as expected BuildKit inputs, and verify both evidence sets against them before admitting the complete Pi environment to the immutable build snapshot and again at the BuildKit boundary before the Dockerfile copies it into `/opt/pi`. Projection resolution remains input-only and side-effect free; a colocated launcher/evidence pair is not trusted without both materialized-attestation values. The GitHub release and SHA256SUMS remain authoritative for input files; lock SRI remains authoritative for dependency bytes.
