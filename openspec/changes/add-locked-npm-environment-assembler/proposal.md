...
- Accept exact root intent plus standard `package-lock.json` v3, validate a strict registry-only subset, preserve validated functional `bin` and `engines.node` metadata separately for every reviewed root by package identity and lock path in the closed model while accepting shape- and syntax-validated transitive engines as ignored npm metadata, and reject unreviewed local, git, link, workspace, traversal, bundled, integrity-less, unsafe executable-path, or incompatible Node-engine inputs.
...
- Maintain a repository-pinned compatibility fixture copied from a published Pi lockfile, with immutable source, Pi version and SHA-256 provenance, to prove the strict parser remains compatible with a real npm-produced closure without replacing synthetic negative and security tests.
