...
- [ ] 1.1 RED — Add failing table-driven tests for valid lockfile-v3 roots, scoped packages, nested placement, integrity, safe `bin` maps and valid `engines.node` ranges for multiple reviewed roots preserved separately in the closed DTO by package identity/path, explicitly accepted-and-ignored `license`, `funding`, and `deprecated` across manifest/reviewed-root/transitive roles, shape-valid transitive `bin`, and syntax-valid manifest/transitive `engines.node` ranges including `||` accepted but absent from semantic DTOs; verify no side-effect-free preflight returns the complete digest-bound validated assembly input contract.
