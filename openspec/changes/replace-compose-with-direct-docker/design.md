...
For `run`, the resolver validates the effective version against the entry's reviewed override policy and selects the artifact catalog entry with that exact version key. Default resolution uses the catalog entry matching the reviewed default version. An override is rejected before projection creation when its exact version has no reviewed catalog entry; the resolver never reuses another version's integrity, synthesizes an artifact URL, or performs network discovery. It then creates a narrow effective runtime projection containing only package identity, effective version, the selected downloadable artifact identity, checksum/integrity, and validation metadata required by the internal installer. Host-only artifact alternatives, update providers, update policy, override policy, and unrelated source fields are omitted even when present in the reviewed runtime section. The generated projection is validated against its own closed DTO schema, written atomically to a private per-launch file under `.docker-generated/runtime/`, and mounted read-only at `/run/pi-cli/docker-constructor.runtime.toml`. A per-launch file prevents concurrent sessions with different overrides from mutating each other's mounted input; the launcher owns cleanup after Docker exits or launch fails. The entrypoint/runtime installer verifies integrity, installs idempotently into mounted Pi home, and validates installed metadata.
