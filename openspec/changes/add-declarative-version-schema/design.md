...
Identical registry input produces byte-identical UTF-8 JSON with stable key ordering and a final newline. Generation contains schema constraints and descriptions but no selected version, URL, revision, or digest copied from `versions.toml`.
...
7. Run offline suites and the real `versions.toml` validation before downstream resolver stages continue.

Rollback is a source revert to the prior inventory validator; `versions.toml` and public model shapes do not require data migration.
