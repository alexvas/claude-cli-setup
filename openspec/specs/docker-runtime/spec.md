...
- **AND** the entrypoint SHALL recursively scan the same directories and only add `g+w` for files and directories that are missing group write permission
...
- **AND** the entrypoint SHALL apply the same selective ownership repair and selective `g+w` repair to `/home/dev/.pi`, `/home/dev/.cargo`, `/home/dev/.npm`, and `/home/dev/.npm-global`
