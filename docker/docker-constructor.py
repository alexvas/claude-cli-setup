#!/usr/bin/env python3
"""
Sole public CLI facade for the Pi Docker constructor.
...
In Stage 8.1 the default dispatcher returns structured
“unavailable” results — no inventory, provider, networking, or
Docker logic is executed.

Usage:
  docker/docker-constructor.py validate [--scope build|runtime|all]
  docker/docker-constructor.py show [--scope build|runtime|all]
                                     [--effective]
  docker/docker-constructor.py check-updates [--scope build|runtime|all]
                                             [--json] [--only FILTER] ...
"""
...
class CommandRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "command_args",
            _deep_freeze(self.command_args),
        )
...
def _deep_freeze(value: object) -> object:
    ...
    * Scalars (str, int, bool, None, …) are returned as-is.
    """
    ...
    if isinstance(value, (dict, MappingProxyType)) or hasattr(value, "items"):
        ...
        return MappingProxyType(
            {k: _deep_freeze(v) for k, v in dict(value).items()}
        )
    return value
...
# ═══════════════════════════════════════════════════════════════════════
# Default (unavailable) dispatcher
# ═══════════════════════════════════════════════════════════════════════
...
def _default_dispatcher(
    ...
    """Return a structured 'unavailable' result for every command."""
    return CommandResult(
        exit_kind=ExitKind.CLI,
        message=f"'{command}' is not available in this stage",
        debug=f"request inventory={request.inventory!r}"
              f" output={request.output!r}"
              f" verbose={request.verbose}"
              f" color={request.color!r}",
    )
    ...
    *,
    ...
    for attr in vars(args):
        if attr in ("command", "func", "inventory", "output", "verbose",
                     "color"):
            continue
        cmd_args[attr] = getattr(args, attr)
...
def main(
    ...
    *,
    ...
    dispatcher:
        ...
        or an object with an ``execute`` method. The default returns
        structured "unavailable" results for every command.
    stdout_isatty:
    ...
    disp = dispatcher or _default_dispatcher
