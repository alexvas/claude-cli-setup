...
def download_bytes(transport: StreamingTransport, url: str) -> bytes:
...
def acquire_install_assets(
    urls: PiReleaseUrls, download: Callable[[str], bytes],
) -> tuple[bytes, bytes]:
    ...
    """
    entries = parse_sha256sums(download(urls.sha256sums))
    package_digest, lock_digest = required_install_digests(entries)
    package = download(urls.install_package)
    lock = download(urls.install_package_lock)
    _verify_digest(package, package_digest, INSTALL_PACKAGE_FILENAME)
    _verify_digest(lock, lock_digest, INSTALL_PACKAGE_LOCK_FILENAME)
    return package, lock
