...
class TestProductionSymlinkHardening(_MaterializationTestCase):
    ...
    def test_permissive_cache_root_fixed_before_download(self) -> None:
        """A pre-existing ``blobs/`` directory with ``0o755`` is
        corrected to ``0o700`` when the descriptor chain opens it."""
        cache_root = self._cache_root
        os.mkdir(cache_root, 0o755)
        self.assertEqual(os.stat(cache_root).st_mode & 0o777, 0o755)
        integrity = _make_integrity_for(b"private-root-bytes")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"private-root-bytes"})
        materialize_selected_artifacts(
            [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
            transport=transport,
    ...
    def test_permissive_intermediate_directory_fixed_before_download(self) -> None:
        """The cache root ``blobs/`` is corrected to ``0o700`` even
        when it lives under a pre-existing permissive parent.
        ``runtime-artifacts/`` itself is NOT altered — enforcement
        is limited to directories inside the cache tree."""
        runtime_dir = os.path.join(self._tmp_root, "runtime-artifacts")
        os.chmod(runtime_dir, 0o755)  # setUp already created it
        cache_root = os.path.join(runtime_dir, "blobs")
        self.assertEqual(os.stat(runtime_dir).st_mode & 0o777, 0o755)
        integrity = _make_integrity_for(b"intermediate-dir")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"intermediate-dir"})
        materialize_selected_artifacts(
            [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
            transport=transport,
            filesystem=LocalCacheFilesystem(),
        ...
        # runtime-artifacts is NOT touched (enforcement is limited).
        self.assertEqual(os.stat(runtime_dir).st_mode & 0o777, 0o755)
        # blobs/ (the cache root) IS fixed.
        self.assertEqual(os.stat(cache_root).st_mode & 0o777, 0o700)
        self.assertEqual(transport.calls, ["https://x.test/pkg.tgz"])

    def test_permissive_lock_root_fixed_before_acquisition(self) -> None:
        """A pre-existing ``locks/`` directory with ``0o755`` is
        corrected to ``0o700`` when the lock factory opens it."""
        cache_root = self._cache_root
        lock_dir = os.path.join(
            os.path.dirname(cache_root), "locks",
        )
        os.mkdir(lock_dir, 0o755)
        self.assertEqual(os.stat(lock_dir).st_mode & 0o777, 0o755)
        integrity = _make_integrity_for(b"lock-dir-private")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"lock-dir-private"})
        materialize_selected_artifacts(
            [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
            transport=transport,
            filesystem=LocalCacheFilesystem(),
    ...
    def test_permissive_container_dir_fixed_before_download(self) -> None:
        """A pre-existing ``.docker-generated/`` directory with
        group/other access is NOT altered — the enforcement is
        limited to the constructor-owned ``runtime-artifacts/``
        tree and below."""
        dot_docker = os.path.join(self._tmp_root, ".docker-generated")
        os.mkdir(dot_docker, 0o755)
        runtime_dir = os.path.join(dot_docker, "runtime-artifacts")
        os.mkdir(runtime_dir, 0o755)
        cache_root = os.path.join(runtime_dir, "blobs")
        self.assertEqual(os.stat(dot_docker).st_mode & 0o777, 0o755)
        self.assertEqual(os.stat(runtime_dir).st_mode & 0o777, 0o755)
        integrity = _make_integrity_for(b"container-dir")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"container-dir"})
        materialize_selected_artifacts(
            [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
            transport=transport,
            filesystem=LocalCacheFilesystem(),
        ...
        # .docker-generated is NOT touched (may contain non-cache content).
        self.assertEqual(os.stat(dot_docker).st_mode & 0o777, 0o755)
        # runtime-artifacts is also NOT touched — enforcement is limited
        # to the cache root (blobs/) and below.
        self.assertEqual(os.stat(runtime_dir).st_mode & 0o777, 0o755)
        # blobs/ (the cache root) IS fixed.
        self.assertEqual(os.stat(cache_root).st_mode & 0o777, 0o700)
        self.assertEqual(transport.calls, ["https://x.test/pkg.tgz"])
    ...
    def test_private_subtrees_secured_after_fresh_materialization(self) -> None:
        ...
        cache_root = os.path.join(runtime_dir, "blobs")
        integrity = _make_integrity_for(b"fresh-subtrees")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"fresh-subtrees"})
        ...
        # ── private subtrees are 0o700 ──────────────────────────
        private_entries = [
            # (path, expected_mode, label)
            (cache_root, 0o700, "blobs"),
            (os.path.join(self._tmp_root, ".docker-generated", "runtime-artifacts", "locks"),
             0o700, "locks"),
        ]
        for p, expected_mode, label in private_entries:
            self.assertEqual(
                os.stat(p).st_mode & 0o777, expected_mode,
                f"{label} ({p!r}) must be {oct(expected_mode)}",
            )
        # Algorithm directory under blobs.
        algo_dirs = [
            d for d in os.listdir(cache_root)
            if os.path.isdir(os.path.join(cache_root, d))
            and not os.path.islink(os.path.join(cache_root, d))
        ]
        self.assertEqual(len(algo_dirs), 1)
        ...
        # 0o777-minus-umask is acceptable either way.
        dot_docker = os.path.join(self._tmp_root, ".docker-generated")
        parent_modes = {
            os.stat(dot_docker).st_mode & 0o777,
            os.stat(runtime_dir).st_mode & 0o777,
        }
        # Both must simply exist — we make no assertion about their
        # exact bits because group-read/write is allowed.
        self.assertGreaterEqual(len(parent_modes), 0)
    ...
    def test_symlinked_cache_root_does_not_alter_external_modes(self) -> None:
        """When the cache root is a symlink to an external directory
        with permissive permissions, that external directory must
        NOT be altered.  The descriptor chain rejects symlinks
        early and never reaches fchmod on the external target."""
        outside_dir = os.path.join(self._tmp_root, "outside-target")
        os.mkdir(outside_dir, 0o755)
        marker = os.path.join(outside_dir, "unchanged")
        with open(marker, "wb") as f:
            f.write(b"do-not-touch")

        # Create a symlink at the nominal cache-root location pointing outside.
        link_path = os.path.join(self._tmp_root, "cache-link")
        os.symlink(outside_dir, link_path)

        integrity = _make_integrity_for(b"symlink-root")
        transport = _FakeTransport({})
        fs = LocalCacheFilesystem()

        # _open_parent / _open_directory_chain reject the symlink.
        with self.assertRaises(ArtifactMaterializationError) as ctx:
            materialize_selected_artifacts(
                [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
                transport=transport,
                filesystem=fs,
                lock_factory=FileIdentityLockFactory(link_path),
                temp_dir=LocalTemporaryDirectory(),
                cache_root=link_path,
            )
        self.assertEqual(ctx.exception.reason, "containment")

        # The external directory and its contents are untouched.
        self.assertEqual(os.stat(outside_dir).st_mode & 0o777, 0o755)
        with open(marker, "rb") as f:
            self.assertEqual(f.read(), b"do-not-touch")
        self.assertEqual(transport.calls, [])

    def test_unfixable_permissive_cache_root_fails_before_download(self) -> None:
        """When the cache root directory has group/other permissions
        but ``fchmod`` fails (e.g. the process lacks ownership),
        materialization must fail with an
        ``ArtifactMaterializationError(reason="permissions")``
        **before** any lock creation, quarantine, algorithm-directory
        creation, network access, or publication."""
        from unittest import mock
        ...
        integrity = _make_integrity_for(b"unfixable")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"unfixable"})
        fs = LocalCacheFilesystem()
        locks = FileIdentityLockFactory(cache_root)
        temp = LocalTemporaryDirectory()

        # Snapshot the entire cache subtree before the call.
        before = self._walk_snapshot(self._tmp_root)

        # Make fchmod fail on the very first call — that is
        # _ensure_dir_private(cache_root), which now runs *before*
        # the fast-path cache check and lock acquisition (step 0).
        ...
        with mock.patch("os.fchmod", side_effect=failing_fchmod):
            with self.assertRaises(ArtifactMaterializationError) as ctx:
                materialize_selected_artifacts(
                    [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
                    transport=transport,
            ...
            self.assertEqual(ctx.exception.reason, "permissions")
            self.assertIn("fchmod denied", str(ctx.exception))

        # ── zero mutation assertions ──────────────────────────────
        # No network access.
        self.assertEqual(transport.calls, [])
        # No lock directory — failure happened before acquire().
        self.assertFalse(
            os.path.exists(os.path.join(self._tmp_root, "locks")),
            "lock directory must not be created",
        )
        # No algorithm directories — ensure_secure_dir never ran.
        self.assertEqual(os.listdir(cache_root), [])
        # Filesystem state is bit-identical to before the call
        # (no quarantine, no publication, no temp files).
        after = self._walk_snapshot(self._tmp_root)
        self.assertEqual(after, before)

    def test_permissive_cache_root_fixed_before_cache_hit(self) -> None:
        """Even on a cache hit (zero-download fast path), the
        cache root must be hardened to ``0o700`` **before** the
        blob is returned, and **no** side effects (lock creation,
        quarantine, new directories, download, or publication)
        may occur."""
        cache_root = self._cache_root
        os.mkdir(cache_root, 0o755)
        algorithm_dir = os.path.join(cache_root, "sha512")

        data = b"cache-hit-root-fix"
        integrity = _make_integrity_for(data)
        _algorithm, _safe, expected_raw = _sri_to_algorithm_digest(integrity)

        # Pre-populate the cache.
        os.mkdir(algorithm_dir, 0o700)
        ...
        # Snapshot the entire cache subtree.
        before = self._walk_snapshot(self._tmp_root)
        ...
        # Root was fixed before the blob was returned.
        self.assertEqual(os.stat(cache_root).st_mode & 0o777, 0o700)
        # Cache hit — no download, no lock dir, no new entries.
        self.assertEqual(transport.calls, [])
        self.assertFalse(
            os.path.exists(os.path.join(self._tmp_root, "locks")),
            "lock directory must not be created on cache hit",
        )
        # Only the permission bits changed; zero new files/dirs.
        after = {
            abspath: (mode & 0o700)  # ignore permission divergence
            for abspath, (mode, _size, _filetype)
            in self._walk_snapshot(self._tmp_root).items()
        }
        before_stripped = {
            abspath: (mode & 0o700)
            for abspath, (mode, _size, _filetype) in before.items()
        }
        self.assertEqual(after, before_stripped)
    ...
    def test_group_writable_parents_preserved_on_cache_miss(self) -> None:
        """Pre-existing ``.docker-generated/`` and
        ...
        runtime_dir = os.path.join(dot_docker, "runtime-artifacts")
        cache_root = os.path.join(runtime_dir, "blobs")
        os.mkdir(dot_docker, 0o770)
        os.mkdir(runtime_dir, 0o770)
        # Force exact mode — os.mkdir honours umask.
        os.chmod(dot_docker, 0o770)
        os.chmod(runtime_dir, 0o770)
        self.assertEqual(os.stat(dot_docker).st_mode & 0o777, 0o770)
        self.assertEqual(os.stat(runtime_dir).st_mode & 0o777, 0o770)

        integrity = _make_integrity_for(b"group-parents-miss")
        transport = _FakeTransport({"https://x.test/pkg.tgz": b"group-parents-miss"})
        result = materialize_selected_artifacts(
            [SelectedArtifact("https://x.test/pkg.tgz", integrity)],
            transport=transport,
            filesystem=LocalCacheFilesystem(),
    ...
    def test_group_writable_parents_preserved_on_cache_hit(self) -> None:
        """Same as cache-miss but using a pre-populated blob so the
        fast cache-hit path is exercised.  Parents must still
        remain untouched and private subtrees must be hardened."""
        dot_docker = os.path.join(self._tmp_root, ".docker-generated")
        runtime_dir = os.path.join(dot_docker, "runtime-artifacts")
        cache_root = os.path.join(runtime_dir, "blobs")
        os.mkdir(dot_docker, 0o770)
        ...
        data = b"group-parents-hit"
        integrity = _make_integrity_for(data)
        _algo, _safe, raw = _sri_to_algorithm_digest(integrity)

        # Pre-populate blob.
        os.mkdir(cache_root, 0o755)  # permissive, will be fixed
        algo_dir = os.path.join(cache_root, _algo)
        os.mkdir(algo_dir, 0o770)  # permissive, will be fixed
        ...
        self.assertEqual(os.stat(algo_dir).st_mode & 0o777, 0o700,
                         "algorithm dir must be 0o700")
        # ── blob returned via cache hit, no download ────────────
        self.assertEqual(transport.calls, [])
        self.assertEqual(len(result), 1)
        actual = list(result.values())[0]
        self.assertEqual(actual.host_path, blob_path)
